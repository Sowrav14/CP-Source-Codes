...
void solve(){
  ...
  // USE __int128 HERE — ONLY CHANGE
  __int128 nn = (__int128)n * n;
  __int128 tmp = nn - (n & 1);
  const int PH = M - 1;

  int n0   = (long long) ((5%PH * nn%PH)%PH);
  int n90  = (long long) (nn%PH + (tmp / 4)%PH)%PH;
  int n180 = (long long) ((2*nn)%PH + (tmp / 2)%PH) % PH;
  int n270 = n90;

  int w0 = bigmod(c, n0, M);
  int w90 = bigmod(c, n90, M);
  int w180 = bigmod(c, n180, M);
  int w270 = bigmod(c, n270, M);
 
  int ans = 0;
  ans = ad(ans, w90, M);
  ans = ad(ans, w180, M);
  ans = ad(ans, w270, M);
  if(n%2) ans = gun(ans, c, M);
  ans = ad(ans, w0, M);
 
  ans = gun(ans, inverse(4, M), M);
  reponse;
}
