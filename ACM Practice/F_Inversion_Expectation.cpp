...
void solve(){
  ...
  for(int i=1;i<=n;i++) if(a[i] != -1) oc[a[i]] = i;
  for(int i=1;i<=n;i++) if(oc[i] == 0) os1.insert(i);
  for(int i=1;i<=n;i++) left[i] = left[i-1] + (a[i] == -1);
  right[n] = a[n] == -1;
  for(int i=n-1;i>=0;i--) right[i] = right[i+1] + (a[i] == -1);

  int tot = os1.size();
  int inv = gun(tot, (tot - 1), M);
  inv = gun(inv, inverse(4, M), M);
  tot = inverse(tot, M);

  for(int i=1;i<=n;i++){
    if(a[i] == -1) continue;

    int x = os2.size() - os2.order_of_key(a[i]);
    inv = ad(inv, x, M);
    os2.insert(a[i]);

    int l = left[i];
    int r = right[i];
    int gt = os1.size() - os1.order_of_key(a[i]);
    int lt = os1.order_of_key(a[i]);

    inv = ad(inv, gun(gun(l, gt, M), tot, M), M);
    inv = ad(inv, gun(gun(r, lt, M), tot, M), M);
  }
