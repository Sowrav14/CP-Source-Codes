...
void allons_y(){
    ...
    int n; cin >> n;
    vector<int>v(n+5, 0), pref(n+5, 0), suf(n+5, 0);
    f1(i,n) cin >> v[i];
    f1(i,n) pref[i] = pref[i-1] + v[i];
    rf1(i,n) suf[i] = suf[i+1] + v[i];
    ...
    // f1(i,n) cout << pref[i] << " "; cout << endl;
    // f1(i,n) cout << suf[i] << " "; cout << endl;

    vector<int>dp(n+5, 0);
    f1(i,n) dp[i] = (suf[i] == s ? 1 : 0);
    rf1(i,n) dp[i] += dp[i+1];

    // f1(i,n) cout << dp[i] << " "; cout << endl;
