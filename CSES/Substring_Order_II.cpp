...
/*
  ...
  suffixes : 
    ...
    aa
    a
  suffix array = [ 5, 4, 1, 2, 3, 0]
  lcp_array = [ 1, 2, 1, 0, 3]

  strings in suffix array look likes(reverse)
    baabaa
    baa
    abaa
    aabaa
    ...
    iterate 
      cur_sa(baabaa) = baabaa nums of substring here = 6
...
void solve(){
	...
	SuffixArray sa(s);
  int n = s.size();
  for(int i=0;i<n;i++) cout << sa.sa[i] << " "; cout << endl;
  for(int i=0;i<n-1;i++) cout << sa.lcp[i] << " "; cout << endl;
}
