...
void solve(){
    ...
    int num = 1, sum = 1, prod = 1, num1 = 1;
    f(i,n){
        num = gun(num, e[i] + 1, mod);
        sum = gun(sum,gun(bigmod(p[i], e[i]+1, mod) - 1, inverse_mod(p[i] - 1, mod), mod) , mod);
    }

    int flag = 1;
    f(i,n){
        if(flag and e[i]%2){
            num1 = gun(num1, (e[i] + 1)/2, phi);
            flag = 0;
        } else{
            num1 = gun(num1, e[i] + 1, phi);
        }
    }
    if(flag) f(i,n) e[i] /= 2;

    f(i,n) prod = gun(prod, bigmod(p[i], e[i], mod), mod);


    prod = bigmod(prod, num1, mod);
