...
void solve(){
    ...
    for(int i=1;i<n;i++){
        pref[i] += pref[i-1] - sa.lcp[i-1];
    }

    int k; cin >> k;
    int it = lower_bound(pref.begin(), pref.end(), k) - pref.begin();
    int x = pref[it] - (it > 0 ? pref[it-1] : 0); // x substrings in sa[it]
    int y = k - (it > 0 ? pref[it-1] : 0); // yth substrings in sa[it] is ans
    int st = sa.sa[it]; // z is the starting index of the suffix in sa[it]
    int l = it > 0 ? sa.lcp[it-1] : 0; // l is the lcp of sa[it] and sa[it-1]
    // cout << y << " " << l << endl;
    int len = y + l;
